...
let sliderMain = new Swiper('.slider', {
	...
	slidesPerGroup: 1,
	navigation: {
		prevEl: '.ourhall__navigation-arrow-left',
		nextEl: '.ourhall__navigation-arrow-right'
	},
	pagination: {
		el: ".ourhall__navigation-range",
		type: 'fraction',
...
let slides = Array.from(document.getElementsByClassName('myslider__slide'));
let mySlider = new Swiper('.myslider__slider', {
	slidesPerView: 5,
	spaceBetween: 105,
	slidesPerGroup: 1,
	initialSlide: 2,
	...
	autoHeight: false,
	navigation: {
		prevEl: '.myslider__arrows-arrow-left',
		nextEl: '.myslider__arrows-arrow-right'
	},
	pagination: {
		el: ".myslider__arrows-range",
		type: 'fraction',
		renderFraction: function (currentClass, totalClass) {
			return ' <span class="' + currentClass + '"></span>' + "из" + '<span class="' + totalClass + '"></span>';
		}
	...
	on: {
		init: function () {
			let activeSlides = this.realIndex;
			if (activeSlides > 0 && activeSlides < (slides.length - 1)) {
				slides[activeSlides - 1].classList.add("blur20");
				slides[activeSlides + 1].classList.add("blur20");
			} else if (activeSlides == 0) {
				slides[activeSlides + 1].classList.add("blur20");
			} else if (activeSlides == (slides.length - 1)) {
				slides[activeSlides - 1].classList.add("blur20");
			};
			if (activeSlides > 1 && activeSlides < (slides.length - 2)) {
				slides[activeSlides - 2].classList.add("blur40");
				slides[activeSlides + 2].classList.add("blur40");
			} else if (activeSlides == 1 || activeSlides == 0) {
				slides[activeSlides + 2].classList.add("blur40");
			} else if (activeSlides == (slides.length - 2) || activeSlides == (slides.length - 1)) {
				slides[activeSlides - 2].classList.add("blur40");
			};
		},
		slideChange: function () {
			...
			slides.forEach(function (item) {
				item.classList.contains('blur20')
				item.classList.remove('blur20');
				item.classList.contains('blur40');
				item.classList.remove('blur40');
			});
			if (activeSlides > 0 && activeSlides < (slides.length - 1)) {
				slides[activeSlides - 1].classList.add("blur20");
				slides[activeSlides + 1].classList.add("blur20");
...
let addElem = document.querySelector(".animate__buttons");
let bodyElem = document.querySelector('.animate__body');
function addElement() {
	addElem.addEventListener("click", (e) => {
		if (e.target.closest('.button__add')) {
			bodyElem.insertAdjacentHTML(
				'afterbegin',
				'<div class="animate__item"> text</div>'
			)
		} else if (e.target.closest('.button__add3')) {
			bodyElem.insertAdjacentHTML(
				'afterbegin',
				'<div class="animate__item"> text</div> <div class="animate__item"> text</div> <div class="animate__item"> text</div>'
			)
		}
